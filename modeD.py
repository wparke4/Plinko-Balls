...
def create_circular_pegs(center_x, center_y, num_layers=7, initial_radius=50, radius_increment=40, initial_pegs=6, peg_increment=6):
    ...
    for i in range(num_layers):
        ...
        for j in range(num_pegs):
            ...
            layer_pins.append({'r': radius, 'angle': angle})
        pins.append(layer_pins)
...
def draw_rotating_wheel(center_x, center_y):
    ...
    for i, layer in enumerate(pins):
        # Determine rotation direction based on layer number
        current_rotation_angle = rotation_angle if (i + 1) % 2 == 1 else -rotation_angle

        for pin in layer:
            # Calculate the new angle by adding the global rotation angle
            current_angle = pin['angle'] + current_rotation_angle
